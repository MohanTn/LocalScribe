...
Within a single compiled backend, *which physical GPU* whisper.cpp uses (e.g. a laptop's integrated + dedicated GPU) is controlled by the `gpuDevice` setting (a device index string, empty = no restriction) via `gpuEnv()` in `whisper.ts`: it sets `CUDA_VISIBLE_DEVICES` for the cuda backend or `GGML_VK_VISIBLE_DEVICES` for vulkan, since whisper.cpp's CLI has no `--device` flag of its own and these runtime env vars are the only lever. `listGpus()` enumerates the choices for the Settings dropdown via `nvidia-smi -L` (cuda) or `vulkaninfo --summary` (vulkan) — since the backend is fixed at compile time, a CUDA-only build can only ever see NVIDIA devices, so a non-NVIDIA integrated GPU won't appear as an option unless whisper.cpp was built with Vulkan (which enumerates GPUs vendor-neutrally). Both the per-job CLI spawn (`whisper.ts`) and the resident `whisper-server` process (`whisper-server.ts`, tracked alongside `currentModel`/`currentForceCpu` so a `gpuDevice` change triggers the same hot-restart) apply this env var.
...
Both `globalShortcut` fallback callbacks (toggle and PTT-as-toggle) are wrapped in `makeFallbackDebouncer()`, which collapses repeated accelerator firings within `FALLBACK_REPEAT_COOLDOWN_MS` (1200ms) into a single call. This exists because Windows' `RegisterHotKey` (what `globalShortcut` uses there) re-fires the accelerator at the OS key-repeat rate for as long as the combo is physically held — Electron registers it without `MOD_NOREPEAT` — so holding either hotkey without this guard storms `onToggle()` and reads as recording flickering on/off instead of a clean start/stop. 1200ms is comfortably above Windows' max configurable initial repeat delay (1000ms via `SPI_GETKEYBOARDDELAY`), so it reliably tells "still the same held key" apart from "a deliberate new press" without querying the OS setting. `uiohookLoader.ts`'s catch block now logs *why* the native module failed to load — check that first when a user reports PTT not truly holding, since it means they're on this fallback path.
